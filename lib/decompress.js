...
module.exports = function(config) {
    ...
    function acceptableCompression(data) {
        ...
        if (data.headers['accept-encoding'] && data.headers['accept-encoding'].includes('gzip')) {
    ...
    function decompressResponse(data) {
        if (config.decompressContentType.indexOf(data.contentType) != -1 && shouldProcess(data)) {
