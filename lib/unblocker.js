...
function Unblocker(config) {
    ...
    if (config.standardMiddleware !== false) {
        ...
        config.handlePywbMiddleware = [
        ].concat(config.handlePywbMiddleware);

        config.decompressContentType = config.decompressContentType || config.processContentTypes;
    }
    ...
    function handleRequest(clientRequest, clientResponse, next) {
        ...
        config.handlePywbMiddleware.map(function(middleware) {
            middleware(clientRequest, clientResponse);
        })
